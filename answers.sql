...
SELECT COUNT(models.name) FROM models WHERE brand_id = 'for';
...
SELECT models.year, models.name, models.model_id, awards.name FROM models LEFT JOIN awards ON models.model_id = awards.winner_id WHERE models.year >= 1964;
...
SELECT brands.name, models.year, models.name
FROM brands
LEFT JOIN models
ON brands.brand_id = models.brand_id
WHERE models.name is NOT NULL
...
SELECT brands.name, models.name, models.year, brands.discontinued, brands.discontinued - models.year As years_until_brand_discontinued
FROM models
LEFT JOIN brands ON models.brand_id = brands.brand_id
WHERE brands.discontinued is NOT NULL
ORDER BY brands.name, models.name, models.year, brands.discontinued;
